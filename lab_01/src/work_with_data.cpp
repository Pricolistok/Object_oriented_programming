...
{
    double y1 = point.y * trigonometry_data.cosX - point.z * trigonometry_data.sinX;
    double z1 = point.y * trigonometry_data.sinX + point.z * trigonometry_data.cosX;

    double x2 = point.x * trigonometry_data.cosY + z1 * trigonometry_data.sinY;
    double z2 = z1 * trigonometry_data.cosY - point.x * trigonometry_data.sinY;
...
int rotate_array_dots(data_points_t &dataset_points, const rotate_param_t &rotateParam)
{
    ...
    if (dataset_points.cnt_points <= 0)
        error_code = ERROR_LEN_DATA;
    else if (!dataset_points.points)
        error_code = ERROR_ADD_MEMORY;
    if (error_code != OK)
        return error_code;
...
int rotate_dots(data_points_t &dataset_points, const rotate_param_t &rotateParam)
{
    ...
    if (!dataset_points.points)
        ...
        error_code = rotate_array_dots(dataset_points, rotateParam);
    return error_code;
}
